...
- **Mock Backend:** `data/employee-registry.json` kart ID + telefon personel eşleştirmesini, `data/users.json` rol, ekip ve uzmanlık bilgisini, `data/tickets.json` talepleri, `data/audit-log.json` işlem izini tutar. `lib/mock-db.ts` ve API route'ları gerçek MVC backend gelene kadar geçici veri katmanıdır.
...
2. **Görünürlük:** Admin ve koordinatör tüm talepleri görür. Teknisyen yalnızca kendi üzerindeki işleri ve yetkili olduğu ekiplerin/uzmanlık alanının kapsamındaki, henüz atanmamış işleri görür. Talep sahibi yalnızca kendi taleplerini görür.
3. **Durumlar:** Sayfalar veri yüklenirken yükleniyor mesajı, API erişiminde uyarı ve işlemlerde Toast sonucu göstermelidir.
4. **Backend sınırı:** Gerçek backend, veritabanı ve SMS/e-posta doğrulaması daha sonra bağlanacaktır. Rol, ekip, uzmanlık ve ticket durum kontrolleri API tarafında da uygulanmalıdır.
