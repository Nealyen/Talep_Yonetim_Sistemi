/**
 * MODAL: Talep Düzenleme — Tüm Talepler sayfasında Admin'in (veya yetkili rollerin)
 * bir talebin tüm alanlarını (durum, atanan kişi, kategori, mesai kaydı vb.)
 * düzenleyebildiği kapsamlı form. forwardRef ile dışarıdan açma/kapama kontrolü alır.
 * En büyük ve en çok sorumluluğu olan modal dosyalarından biridir (500+ satır).
 */
...
const createInitialForm = (ticket: Ticket | null, categoryTree: Record<string, string[]>): EditFormState => {
...
export const TicketEditModal = forwardRef<TicketEditModalHandle, TicketEditModalProps>(({ visible, ticket, onHide, onSave, footer }, ref) => {
  ...
  // KURAL: Kategori ağacı artık statik değil, Kategori Yönetimi sayfasından (Admin)
  // canlı olarak besleniyor. Burada kasıtlı olarak "pasif" filtrelenmiyor — bir talep
  // zaten pasif hale getirilmiş bir kategoriyle oluşturulmuş olabilir, düzenleme
  // ekranında o seçimin kaybolmaması/görünmeye devam etmesi gerekir. Pasif olanlar
  // sadece "Yeni Talep Oluştur" formunda gizlenir.
  const { ustBasliklar, items } = useCategories();
  const categoryTree = useMemo(() => {
    const tree: Record<string, string[]> = {};
    ustBasliklar.forEach((ust) => {
      tree[ust] = items.filter((i) => i.ustBaslik === ust).map((i) => i.altBaslik);
    });
    return tree;
  }, [ustBasliklar, items]);
  const mainCategoryOptions = useMemo(() => Object.keys(categoryTree).map((key) => ({ label: key, value: key })), [categoryTree]);

  const [editForm, setEditForm] = useState<EditFormState>(createInitialForm(ticket, categoryTree));
  ...
  useEffect(() => {
    if (!ticket) {
      const emptyForm = createInitialForm(null, categoryTree);
    ...
    const baseForm = createInitialForm(ticket, categoryTree);
