...
/**
 * HOOK: Teknik İş Havuzu sayfasının state ve iş mantığı.
 * KURAL — GÖRÜNÜRLÜK: Bir talep belirli bir ekibe eşlenmişse (Kategori Yönetimi'nde
 * tanımlı), havuzda SADECE o ekipteki teknisyenlere görünür (bkz. isVisibleToMe).
 * Ekibi atanmamış (boş) talepler herkese açık kalır. Admin/Koordinatör her zaman tüm
 * havuzu görür.
 */
...
export const useIsHavuzu = () => {
    ...
    // KURAL: Bir talep belirli bir ekibe eşlenmişse (Kategori Yönetimi'nde tanımlanmış),
    // bu talebi havuzda yalnızca o ekipte olan TEKNİSYENLER görebilir — "her rol her işi
    // görmesin" kuralı. ADMIN ve KOORDİNATÖR gözetim rolü olduğu için tüm havuzu görmeye
    // devam eder (zaten "Tüm Talepler" ve "Süreç Takibi" sayfalarında da sınırsız erişimleri var).
    // Bir talebin ekibi henüz atanmamışsa (boş), herkese açık kalır.
    const isVisibleToMe = (ticket: Ticket) => {
        if (currentUser.role === 'ADMIN' || currentUser.role === 'KOORDINATOR') return true;
        if (!ticket.team) return true;
        return (currentUser.teams || []).includes(ticket.team);
    };

    const visibleUnassignedTickets = unassignedTickets.filter(isVisibleToMe);

    const filteredTickets = visibleUnassignedTickets.filter(
        (ticket) =>
    ...
    const handleTakeOwnership = async (ticket: Ticket) => {
        ...
        if (!isVisibleToMe(ticket)) return;
