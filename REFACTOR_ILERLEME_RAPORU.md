...
## 🆕 GÜNCELLEME (4. tur) — Kritik tarih ayrıştırma hatası ve Ekip Yönetimi onayları

1. **KRİTİK DÜZELTME — `parseTurkishDate` (tüm projeyi etkiliyordu):** `utils/ticketHelpers.ts`
   içindeki bu fonksiyon, "02.09.2026 10:04" gibi TR formatlı tarihleri önce
   JavaScript'in yerleşik `new Date(value)`'una veriyordu. Bu, hataya düşmeden ama
   **YANLIŞ** bir tarih üretiyordu (gün.ay.yıl'ı ay.gün.yıl sanıyordu — örn. 2 Eylül'ü
   9 Şubat gibi yorumluyordu). Bu sessiz hata, "Son 1 Ay/Son 3 Ay" dashboard
   grafiklerinin, Taleplerim'in tarih filtresinin ve Geçmiş Talepler'in tarih
   filtresinin hep boş/yanlış çıkmasına sebep oluyordu. Artık native `Date` sadece
   gerçek ISO formatı için kullanılıyor, TR formatındaki tüm tarihler elle
   (gün/ay/yıl sırasına saygılı şekilde) ayrıştırılıyor.

2. **Ekip Yönetimi'ne silme onayı eklendi:** Hem "Ekipler" listesindeki bir ekibi
   silmek hem de bir personel-ekip eşleşmesini silmek artık önce onay penceresi
   çıkarıyor ("Emin misiniz?"). Ekip silinirken, o ekibe kaç personelin atanmış
   olduğu da mesajda gösteriliyor.

3. **Kozmetik TypeScript uyarısı giderildi:** `types/layout.d.ts` içindeki
   `AppMenuItemProps` tipine `key?: string;` eklendi. Bu, React'in liste
   render ederken zorunlu tuttuğu (çalışma zamanında hiçbir etkisi olmayan)
   `key` prop'unu TypeScript'in de tanıması içindi — projenin en başından beri
   `layout/AppMenu.tsx` ve `layout/AppMenuitem.tsx`'te çıkan 2 kırmızı tip
   hatasını temizledi. Uygulamanın çalışmasında hiçbir değişikliğe yol açmaz.
...
- `yonerge`, `landing/page.tsx` (557 satır), auth/404 sayfaları — hiç incelenmedi.
  (`sss` sayfası kontrol edildi, hiçbir yerde kullanılmadığı için silindi.)
...
## 🆕 GÜNCELLEME (5. tur) — Kullanıcılar ve Ekip Yönetimi iyileştirmeleri

1. **Kullanılmayan `sss` sayfası silindi:** Menüde veya kodun hiçbir yerinde bu
   sayfaya bağlantı yoktu (`grep` ile doğrulandı), tamamen ölü/erişilemez bir
   rota olduğu için `app/(main)/sss/` klasörü kaldırıldı.

2. **Kullanıcılar tablosu — Ekip kolonu:** CALISAN rolündeki kullanıcılar için
   (ekip ataması zaten yapılmadığı için) "Ekip Atanmadı" yerine sade bir "-"
   gösteriliyor. Diğer roller için "Ekip Atanmadı" bir hatırlatma olarak kalmaya
   devam ediyor.

3. **Kullanıcılar tablosundan hızlı ekip ekleme/çıkarma:** Her satırın Ekip
   kolonuna bir "+" butonu eklendi — tıklanınca küçük bir açılır pencerede
   (henüz atanmamış) ekipler listeleniyor, seçilince direkt ekleniyor. Mevcut
   ekip etiketlerinin üzerine gelince sağında bir "x" beliriyor (imleç
   ayrılınca kayboluyor), tıklanınca o ekip o kişiden kaldırılıyor — ayrı bir
   sayfaya gitmeden, tablonun içinden hızlıca yönetilebiliyor. (Detaylı/toplu
   yönetim hâlâ "Ekip Yönetimi" sayfasında.)

4. **Ekip Yönetimi — "Birim Personeli" tablosuna onay adımı eklendi:** Önceden,
   yeni bir satırda kişi + ekip seçilir seçilmez otomatik olarak kaydediliyordu.
   Artık ikisi de seçildiğinde satırda çöp kutusu yerine bir ✓ (onay) ikonu
   çıkıyor; kayıt ancak bu ikona tıklanınca kalıcı oluyor (o zaman çöp kutusuna
   dönüşüyor). Taslak (henüz onaylanmamış) bir satırı iptal etmek için hâlâ
   direkt silinebiliyor, onay gerekmiyor.

5. **Aynı kişi aynı ekibe iki kez eklenemez:** Hem yeni satır onaylanırken hem
   de var olan bir satırda kişi/ekip değiştirilirken, seçilen kişi zaten o
   ekibe dahilse artık sessizce yok sayılmıyor — kırmızı bir hata mesajı
   gösterip işlemi iptal ediyor.
