/**
 * DOSYA: Dosya tabanlı (JSON) sahte/mock veritabanı katmanı.
 * DİKKAT: Bu, GERÇEKTE KULLANILAN state yönetimi DEĞİLDİR — asıl uygulama
 * layout/context/*.tsx context'leri + localStorage ile çalışır. Bu dosya sadece
 * app/api/* altındaki (çoğunlukla kullanılmayan, bkz. o dosyalardaki notlar) eski/
 * prototip API route'larını besler. Aktif olarak kullanılan tek bağlantı: /api/audit
 * (hooks/useAuditLogs.ts üzerinden) ve /api/upload (sahte dosya yükleme).
 */
